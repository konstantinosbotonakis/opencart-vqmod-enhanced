...
abstract class VQMod {
	...
	public static function bootup($path = false, $logging = true) {
		...
		$logs_folder 	= $vqmod_folder . 'logs/';
		$caches_folder 	= $vqmod_folder . 'vqcache/';
		$mods_cache 	= $vqmod_folder . 'mods.cache';
		$checked_caches = $vqmod_folder . 'checked.cache';
		$protected_filelist = $vqmod_folder . 'vqprotect.txt';
		$path_replaces 	= $vqmod_folder . 'pathReplaces.php';

		if ($logs_folder !== self::$logFolder) {
			self::$logFolder = $logs_folder;
		}
		if ($caches_folder !== self::$vqCachePath) {
			self::$vqCachePath = $caches_folder;
		}
		if ($mods_cache !== self::$modCache) {
			self::$modCache = $mods_cache;
		}
		if ($checked_caches !== self::$checkedCache) {
			self::$checkedCache = $checked_caches;
		}
		if ($protected_filelist !== self::$protectedFilelist) {
			self::$protectedFilelist = $protected_filelist;
		}
		if ($path_replaces !== self::$pathReplaces) {
			self::$pathReplaces = $path_replaces;
		}
	...
	private static function _getMods() {
		...
		self::$_modFileList = glob(self::path($vqmod_folder . '/xml/', true) . '*.xml');
